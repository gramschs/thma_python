...
EVA-Prinzip. Welche Eingaben sind möglich (Argumente und Parameter der Funktion)
und was gibt de Funktion nach der Verarbeitung als Ausgabe zurück.

## Scatterplots mit Plotly Express

Das erste Argument der Funktion ist `data_frame`, in der Standardeinstellung ist
`data_frame=None` gesetzt. Wir brauchen also das erste Argument nicht
notwendigerweise. `None` steht für einen fehlenden oder leeren Wert. Das zweite
Argument ist `x=None` und das dritte Argument `y=None`. Auch diese Argumente
sind optional. In der Beschreibung der Parameter steht, dass `x` und `y` Listen
sein können. Probieren wir es aus und definieren wir eine Liste mit x- und
y-Koordinaten.

...
Datenstruktur, die ähnlich wie eine Liste mehrere Werte zusammenfasst, aber ein
Tupel ist im Gegensatz zu einer Liste unveränderlich. Bei einem eindimensionalen
Array enthält das Tupel nur einen Wert: die Anzahl der Elemente. `.dtype` zeigt
den Datentyp aller Elemente an. Enthält mindestens eine Zahl einen Dezimalpunkt,
wählt NumPy automatisch den Typ `float64`:

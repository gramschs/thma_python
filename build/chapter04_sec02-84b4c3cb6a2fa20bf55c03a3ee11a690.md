...
Zum Abschluss des Kapitels wenden wir die if-Verzweigung zum Stylen eines
...
Wir möchten dem Benutzer oder der Benutzerin die Wahl der Sprache bei Titel und
die Achsenbeschriftung lassen. Voreingestellt ist deutsch (=DE). Aber wenn
Englisch (=EN) gewählt wird, soll der Titel "Brake disc temperature during
braking" lauten und die Achsenbeschriftung "Time in seconds" bzw. "Temperature
in °C".

```{admonition} Mini-Übung
:class: tip
Initialisieren Sie drei Variablen mit den deutschen Texten
"Bremsscheibentemperatur während der Bremsung", "Zeit in Sekunden" und
"Temperatur in °C". Lassen Sie dann mit `input()` nach der bevorzugten Sprache
fragen. Wenn die Antwort "EN" ist, lassen Sie Ihr Skript die deutschsprachigen
Texte durch die englischsprachigen Texte "Brake disc temperature during braking"
sowie "Time in seconds" bzw. "Temperature in °C" ersetzen. Bei allen anderen
Antworten soll die deutsch Voreinstellung belassen werden.

Lassen Sie zuletzt das Liniendiagramm anzeigen.
```

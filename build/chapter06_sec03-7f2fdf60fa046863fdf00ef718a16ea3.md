...
mit Leerzeichen auf. Das `d` steht für "decimal integer".
````
...
1. Erweitern Sie den Code so, dass Reaktionsweg und Anhalteweg berechnet
   werden.
2. Formatieren Sie die Ausgabe mit f-Strings so, dass:
   - die Geschwindigkeit als Ganzzahl in einem Feld der Breite 3 steht,
   - der Bremsweg auf 1 Nachkommastelle gerundet in einem Feld der Breite 6
     steht,
   - der Reaktionsweg auf 1 Nachkommastelle gerundet in einem Feld der Breite
     6 steht,
   - der Anhalteweg auf 1 Nachkommastelle gerundet in einem Feld der Breite 7
     steht.
...
for v in range(30, 131, 10):
    ...
    print(f"v = {v:3d} km/h,  Bremsweg = {bremsweg:6.1f} m,  "
          f"Reaktionsweg = {reaktionsweg:6.1f} m,  Anhalteweg = {anhalteweg:7.1f} m")
```
...
Hinweis: Ein f-String darf über mehrere Zeilen verteilt werden, indem man ihn
...
2. Geben Sie das letzte und das vorletzte Zeichen mit negativem Index aus.
   Was stellen diese beiden Zeichen zusammen dar?
3. Bestimmen Sie die Länge des Strings mit `len()`.
4. Prüfen Sie mit einer if-Bedingung, ob das Trennzeichen an Position 4
   (Index 4) ein `"-"` ist, und geben Sie in diesem Fall
   `"Gültiges Datumsformat erkannt."` aus.
5. Versuchen Sie, das Jahr auf `"2025"` zu ändern, indem Sie
   `datum[0] = "2"` ausführen. Was passiert? Warum?
6. Erzeugen Sie stattdessen durch String-Verkettung einen neuen String
   `"2025-03-15"` und geben Sie ihn aus. Verwenden Sie dabei den Teil
   des ursprünglichen Strings ab Index 4.

Hinweis zu Teilaufgabe 6: `datum[4:]` ist ein sogenannter Slice und liefert
alle Zeichen ab Index 4 bis zum Ende des Strings. Das Slicing wird in einem
späteren Kapitel ausführlich behandelt.
````
...
# 5. Schreibzugriff schlägt fehl
# datum[0] = "2"   # TypeError: 'str' object does not support item assignment

# 6. Neuen String erzeugen
datum_neu = "2025" + datum[4:]
print(datum_neu)      # 2025-03-15
```
...
Erklärung zu Teilaufgabe 5: Strings sind in Python **unveränderlich**. Einzelne
Zeichen können nicht nachträglich ausgetauscht werden. Der Versuch führt zu
einem `TypeError`. Um einen String zu verändern, muss immer ein neuer String
erzeugt werden, zum Beispiel durch Verkettung. `datum[4:]` liefert alle Zeichen
ab Index 4, also `"-03-15"`, und wird mit `"2025"` zur neuen Datumsangabe
zusammengesetzt.
````
...
Simulieren Sie 200 Drehungen des Glücksrads mit `np.random.uniform(0, 10)`.

1. Zählen Sie, wie viele der 200 Drehungen in der Gewinnzone landen.
2. Berechnen Sie die Gewinnquote in Prozent.
3. Geben Sie die Ergebnisse mit f-Strings aus:
   - `"Gewinnzone:    58 von 200 Drehungen"`
   - `"Gewinnquote:   29.0 %"`
4. Visualisieren Sie die 200 Zufallswerte als Histogramm mit `px.histogram()`.
...
# Ausgabe
print(f"Gewinnzone:   {anzahl_gewinne:3d} von {ANZAHL_DREHUNGEN} Drehungen")
print(f"Gewinnquote:  {gewinnquote:.1f} %")

fig = px.histogram(x=werte,
                   labels={"x": "Ergebnis", "y": "Häufigkeit"},
...
1. Simulieren Sie mit je einer for-Schleife 500 gefertigte Bolzen pro Maschine.
   Verwenden Sie `np.random.normal()` mit dem jeweiligen Mittelwert und der
   jeweiligen Standardabweichung.
2. Visualisieren Sie die Durchmesserverteilung beider Maschinen als je ein
   Histogramm mit `px.histogram()`.
3. Beschreiben Sie in einem Kommentar im Code, was Sie in den beiden
...
# Beobachtung:
# Das Histogramm von Maschine A zeigt eine sehr schmale, hohe Glockenform.
# Fast alle Werte liegen sehr nahe am Sollwert 20.0 mm.
# Das Histogramm von Maschine B ist deutlich breiter und flacher:
# Die Werte streuen stark um den Sollwert, was auf viel Ausschuss hindeutet.
# Beide Histogramme sind um denselben Mittelwert 20.0 mm zentriert.
```

Der direkte Vergleich der Histogramme macht den Einfluss der
Standardabweichung unmittelbar sichtbar: Eine kleinere Standardabweichung
bedeutet eine schmalere Glocke und damit einen präziseren Fertigungsprozess
...
1. Geben Sie den aktuellsten und den ältesten Messwert mit negativem Index
   bzw. positivem Index aus.
2. Berechnen Sie den Mittelwert der bisherigen Messungen mit einer for-Schleife
   (ohne `sum()` oder `mean()`).
3. Geben Sie mit einem f-String aus:
   `"Aktuellster Wert: 23.0 °C  |  Ältester Wert: 18.2 °C  |  Mittelwert: 21.1 °C"`
...
fig = px.histogram(x=simulierte_werte,
                   labels={"x": "Temperatur in °C", "y": "Häufigkeit"},

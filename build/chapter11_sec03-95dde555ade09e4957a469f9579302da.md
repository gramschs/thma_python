...
3. Erstellen Sie ein Streudiagramm mit `Kraft_N` auf der x-Achse und
   `Verlaengerung_mm` auf der y-Achse. Färben Sie die Punkte nach `Werkstoff`
   ein. Was beobachten Sie?
````
...
fig = px.scatter(data_frame=df,
                 ...
                 y="Verlaengerung_mm",
                 color="Werkstoff",
                 title="Zugversuch: Verlängerung in Abhängigkeit der Kraft")
fig.show()
```
...
einen linearen Verlauf zeigen. Aluminium hat die steilste Kennlinie, da es den
niedrigsten E-Modul hat und sich bei gleicher Kraft am stärksten verlängert.
Stahl hat die flachste Kennlinie.
````
...
df["Dehnung_Proz"] = df["Verlaengerung_mm"] / 100 * 100
...
1. Filtern Sie den Datensatz auf den Werkstoff `"Stahl"` und extrahieren Sie
   `Kraft_N` und `Verlaengerung_mm` als NumPy-Arrays.
...
3. Berechnen Sie mit `np.linspace()` ein Werte-Array `x_fit` vom minimalen
   bis maximalen Kraftwert (200 Punkte) und die zugehörigen Vorhersagewerte
   `y_fit`.
...
x = df_stahl["Kraft_N"].values
y = df_stahl["Verlaengerung_mm"].values
...
# Ausgabe
print(f"Steigung: {koeff[0]:.2e} mm/N")
print(f"R²:       {R2:.4f}")
...
Gesetz im elastischen Bereich entspricht. Die Steigung von etwa 9.5 · 10⁻⁶ mm/N
beschreibt die Nachgiebigkeit der Probe: Pro Newton Kraft verlängert sie sich
um diesen Betrag.
````
...
`"Titan"` die Steigung der Regressionsgeraden und R². Geben Sie die Ergebnisse
als formatierte Tabelle aus:

```
Werkstoff  |  Steigung (mm/N)  |  R²
-----------|-------------------|-------
Stahl      |      9.52e-06     | 0.9958
Aluminium  |      ...          | ...
Titan      |      ...          | ...
```
...
print(f"{'Werkstoff':<12} | {'Steigung (mm/N)':>17} | {'R²':>6}")
print("-" * 42)

for mat in ["Stahl", "Aluminium", "Titan"]:
    sub = df[df["Werkstoff"] == mat]
    x = sub["Kraft_N"].values
    y = sub["Verlaengerung_mm"].values
    koeff = np.polyfit(x, y, 1)
    R2 = np.corrcoef(x, y)[0, 1]**2
    print(f"{mat:<12} | {koeff[0]:>17.2e} | {R2:.4f}")
```

Aluminium hat die größte Steigung, da es den niedrigsten E-Modul der drei
Werkstoffe besitzt. Eine große Steigung bedeutet, dass sich die Probe bei
gleicher Kraft stärker verlängert. Mit anderen Worten: Aluminium ist unter
denselben Bedingungen nachgiebiger als Stahl oder Titan.
````

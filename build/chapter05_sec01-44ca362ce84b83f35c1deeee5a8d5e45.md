...
nass / vereist) oder dem Fahrzeugtyp (inbesondere Leistung der Bremse). Mit
einer for-Schleife und `range()`erstellen wir eine Tabelle mit Bremswegen.

Beispiel: Ausgabe einer Tabelle mit Berechnung von Bremswegen abhängig von der
Geschwindigkeit

```{code-cell} python
for v in range(30, 131, 10):
    bremsweg = v**2 / 250  # vereinfachte Formel
    print("v =", v, "km/h,  Bremsweg =", bremsweg, "m")
```

```{admonition} Mini-Übung
:class: tip
Schreiben Sie ein Programm, dass die Summe der ersten 100 Quadratzahlen berechnet.
```
...
print("Die Summe der ersten 100 Quadratzahlen ist:", summe)
```
````

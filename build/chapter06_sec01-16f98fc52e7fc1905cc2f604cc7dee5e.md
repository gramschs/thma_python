...
```{dropdown} Video "Indizes in Python" von Programmieren Starten
<iframe width="560" height="315" src="https://www.youtube.com/embed/AGZT2TmEcGY"
...
einen Index, wobei jedes Zeichen eine Position belegt. Der String `"Welle"` hat
fünf Zeichen, die an den Positionen 0 bis 4 (bzw. -1 bis -5) gespeichert sind.
...
Die Funktion `len()`, die wir aus Kapitel 3 für Listen kennen, funktioniert also
...
2. Bestimmen Sie die Länge des Strings mit `len()`.
3. Versuchen Sie, das letzte Zeichen durch `"2"` zu ersetzen. Was passiert?
4. Erzeugen Sie stattdessen einen neuen String `"Antriebswelle_v2"` durch
   String-Verkettung. Hinweis: Greifen Sie mit einem Slice `bauteil[:-1]` auf
   alle Zeichen bis auf das letzte zu.
```
...
# 3. Schreibzugriff schlägt fehl
# bauteil[-1] = "2"  # TypeError!

# 4. Neuen String erzeugen
bauteil_neu = bauteil[:-1] + "2"
print(bauteil_neu)   # Antriebswelle_v2
```

`bauteil[:-1]` ist ein sogenannter **Slice** und liefert alle Zeichen vom
Anfang bis zum (aber nicht einschließlich) letzten Zeichen. Das Slicing wird in
einem späteren Kapitel ausführlicher behandelt.
````

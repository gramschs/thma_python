...
stehen. Der folgende Code führt zu einem Fehler:
...
der Spalte `Farbe` 20 Werte. Das sind weniger als 0.1 Prozent aller Zeilen, was
für einen realen Datensatz sehr wenig ist.

Für viele Auswertungen müssen fehlende Werte entfernt werden, da sie
Berechnungen wie `.mean()` verfälschen oder Fehler verursachen können. Die
Methode `.dropna()` entfernt Zeilen mit fehlenden Werten. Mit dem Parameter
`subset` schränken wir das auf eine bestimmte Spalte ein, sodass nur Zeilen
entfernt werden, in denen genau diese Spalte fehlt:

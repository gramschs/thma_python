...
:class: warning
`np.polyfit()` erwartet NumPy-Arrays ohne fehlende Werte. Aus einem DataFrame
extrahieren wir die Spalten daher mit `.values` und entfernen vorher alle Zeilen
mit fehlenden Werten in den relevanten Spalten mit `.dropna(subset=[...])`.
```

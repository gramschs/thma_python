...
vorliegen. Realisieren Sie einen einfachen Sprach-Umschalter für das
Liniendiagramm der Bremsscheibentemperatur:

* Initialisieren Sie Variablen für Titel und Achsenbeschriftungen mit den
  deutschen Texten.
* Fragen Sie den Benutzer oder die Benutzerin mit `input()` nach der gewünschten
  Sprache (`DE` oder `EN`).
* Wenn `EN` eingegeben wird, sollen die Texte durch die englischen
  Entsprechungen ersetzt werden.
* Zeigen Sie abschließend das Liniendiagramm mit den gewählten Texten an.

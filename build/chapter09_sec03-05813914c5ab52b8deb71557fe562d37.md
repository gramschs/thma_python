...
`fig.add_scatter()`. Speichern Sie die Datenreihen vorher in einem Dictionary,
um den Zusammenhang zwischen Name und Werten klar zu machen:

```python
import plotly.express as px

serien = {"Reihe 1": y_werte_1, "Reihe 2": y_werte_2}

fig = px.line(x=x_werte, y=serien["Reihe 1"],
              title="Titel")
fig.data[0].name = "Reihe 1"
fig.add_scatter(x=x_werte, y=serien["Reihe 2"], name="Reihe 2")
fig.show()
```
...
serien = {
    "200 N/m":  kraefte_weich,
    "500 N/m":  kraefte_mittel,
    "1000 N/m": kraefte_steif
}

# Ausgabe
fig = px.line(x=auslenkungen, y=serien["200 N/m"],
              labels={"x": "Auslenkung (m)", "y": "Federkraft (N)"},
              title="Federkennlinien im Vergleich")
fig.data[0].name = "200 N/m"
fig.add_scatter(x=auslenkungen, y=serien["500 N/m"],  name="500 N/m")
fig.add_scatter(x=auslenkungen, y=serien["1000 N/m"], name="1000 N/m")
fig.show()
```

Erklärung: Das Dictionary `serien` ordnet jedem Namen die zugehörige
Werteliste zu. Der Zugriff `serien["200 N/m"]` gibt die entsprechende Liste
zurück. So bleibt der Zusammenhang zwischen Name und Daten im Code sichtbar,
ohne dass die Variablennamen separat verwaltet werden müssen.
````
...
# Ausgabe: Diagramm
wege = {
    "Reaktionsweg": reaktionswege,
    "Bremsweg":     bremswege,
    "Anhalteweg":   anhaltewege
}

fig = px.line(x=geschwindigkeiten, y=wege["Reaktionsweg"],
              labels={"x": "Geschwindigkeit (km/h)", "y": "Weg (m)"},
              title="Anhaltewege in Abhängigkeit der Geschwindigkeit")
fig.data[0].name = "Reaktionsweg"
fig.add_scatter(x=geschwindigkeiten, y=wege["Bremsweg"],   name="Bremsweg")
fig.add_scatter(x=geschwindigkeiten, y=wege["Anhalteweg"], name="Anhalteweg")
fig.show()
```
...
# Verarbeitung
leistungen = {
    "Elektromotor":         berechne_leistung_elektrisch(400, 25),
    "Hydraulikzylinder":    berechne_leistung_hydraulisch(15_000_000, 0.002),
    "Antriebswelle":        berechne_leistung_mechanisch(800, 5),
    "Kühlwasserkreislauf":  berechne_leistung_thermisch(2.0, 4182, 5)
}

# Ausgabe: Tabelle
print(f"{'Szenario':<22}  {'Leistung':>12}")
print("-" * 36)
for name in leistungen:
    print(f"{name:<22}  {leistungen[name]:>10.1f} W")

# Ausgabe: Balkendiagramm
fig = px.bar(x=list(leistungen.keys()),
             y=list(leistungen.values()),
...
Erklärung: Das Dictionary `leistungen` verknüpft jeden Szenarionamen direkt
mit dem berechneten Leistungswert. Die for-Schleife über ein Dictionary
iteriert automatisch über alle Schlüssel. Mit `leistungen[name]` wird der
zugehörige Wert abgerufen. `list(leistungen.keys())` und
`list(leistungen.values())` übergeben Szenarionamen und Leistungswerte als
Listen an Plotly Express.
````

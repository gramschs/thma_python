...
`.show()` erzwingen wir die Anzeige explizit. Dazu speichern wir das Diagramm in
einer Variablen.
...
Jetzt fehlt nur noch ein Titel für das Diagramm. Die Hilfe verrät uns, dass dem
Parameter `title` ein String übergeben werden kann. Wir setzen das Argument auf
den Titel Rechteck.

```{code-cell} python
# Ausgabe als Lineplot mit Titel
fig = px.line(x=x_koordinaten, y=y_koordinaten, title='Rechteck')
fig.show()
```

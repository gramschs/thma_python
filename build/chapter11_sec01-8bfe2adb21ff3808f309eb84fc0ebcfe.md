...
filtern ihn auf die vier häufigsten Kraftstoffarten sowie auf Preise bis
150.000 Euro, um Ausreißer in der Visualisierung zu vermeiden:
...
Punkt automatisch nach einer kategorialen Spalte ein und fügt eine Legende
hinzu, ohne dass wir selbst gruppieren müssen.
...
```{admonition} Aufbau eines Boxplots
:class: note
...
* Die **Whisker** (Antennen) erstrecken sich bis zum kleinsten bzw. größten Wert
  innerhalb des 1.5-fachen IQR.
...
Standardmäßig stapelt Plotly Express die Balken (`barmode="stack"`). Für einen
...
```{admonition} `barmode="overlay"` vs. `barmode="group"`
:class: note

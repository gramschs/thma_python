...
englische Begriff **Integer** verwendet. Mit Integern können wir rechnen, also
...
Sobald wir mathematisch gesehen eine Division vorliegen haben, die nicht aufgeht
wie beispielsweise 1/2, verlassen wir den Bereich der ganzen Zahlen und kommen
automatisch zu den rationalen Zahlen. Die rationalen Zahlen werden in der
Informatik als Fließkommazahl bzw. als **Float** bezeichnet. Python verwendet
für das Ergebnis einer Division immer einen Float, unabhängig davon, ob die
Division ganzzahlig durchführbar wäre oder nicht, wie wir hier sehen:
...
werden. Auch wenn das zunächst unerwartet ist, ist die Verkettung sehr nützlich,
wenn wir einen String aus mehreren Strings zusammensetzen möchten:
beispielsweise Zahlen mit Einheit auszugeben:
...
:class: tip
Schreiben Sie in die nächste Code-Zelle einfach den Buchstaben `n` unter die
Kommentarzeile und lassen Sie dann die Code-Zelle mit `run` vom
Python-Interpreter ausführen. Was beobachten Sie? Recherchieren Sie im Internet
...
n
```
Der Interpreter zeigt in rot eine Fehlermeldung an: "NameError: name 'n' is not
defined". Damit weist der Interpreter darauf hin, dass die Variable bisher nicht
mit einem Wert versehen wurde, sie ist nicht intialisiert worden. Daher kann
damit auch nicht gearbeitet werden.
````
...
beispielsweise eingebaute Kommandos für den Python-Interpreter. Würden Sie diese
...
In diesem Kapitel haben wir uns mit der Verarbeitung (Datentypen und Speichern
...
von datenverarbeitenden Systemen kennenlernen: Eingabe - Verarbeitung - Ausgabe.
